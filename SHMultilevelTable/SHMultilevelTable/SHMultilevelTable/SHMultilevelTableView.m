...
@interface SHMultilevelTableView () <UITableViewDelegate,UITableViewDataSource>


@end

@implementation SHMultilevelTableView
...
{
    if ( [self.delegate respondsToSelector:@selector(numberOfSectionsInMutilevelTableView:)] ){
        return [self.delegate numberOfSectionsInMutilevelTableView:self];
    }
    return 1;
}
...
- (NSInteger) totalNumberOfRowsInNodePath:(SHMultilevelNodePath *) nodePath
{
    ...
    //获取子节点数
    if ([self.delegate respondsToSelector:@selector(mutilevelTableView:numberOfRowsInNodePath:)])
        indexPathNum = [self.delegate mutilevelTableView:self numberOfRowsInNodePath:nodePath];
...
- (NSInteger) totalNumberOfRowsInSection:(NSInteger) section{
    ...
    if ([self.delegate respondsToSelector:@selector(mutilevelTableView:numberOfRowsInSection:)])
        sectionNum = [self.delegate mutilevelTableView:self numberOfRowsInSection:section];
...
- (SHMultilevelNodePath *) subNodeWithRootMutilevelNodePath:(SHMultilevelNodePath *)nodePath row:(NSInteger) row
{
    ...
    if ([self.delegate respondsToSelector:@selector(mutilevelTableView:numberOfRowsInNodePath:)])
        subNodeNum = [self.delegate  mutilevelTableView:self numberOfRowsInNodePath:nodePath];
...
- (SHMultilevelNodePath *) conversionToNodePathWithIndexPath:(NSIndexPath *)indexPath
{
    ...
    //获取当前section又多少个根节点
    if ([self.delegate respondsToSelector:@selector(mutilevelTableView:numberOfRowsInSection:)])
        sectionNum = [self.delegate  mutilevelTableView:self numberOfRowsInSection:indexPath.section];
...
- (CGFloat) tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(mutilevelTableView:heightForRowAtNodePath:)])
        return  [self.delegate mutilevelTableView:self heightForRowAtNodePath:nodePath];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(mutilevelTableView:cellForRowAtNodePath:)])
        cell = [self.delegate mutilevelTableView:self cellForRowAtNodePath:nodePath];
    if (!cell) {
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(mutilevelTableView:didSelectRowAtIndexPath:openNode:)])
        [self.delegate mutilevelTableView:self didSelectRowAtIndexPath:nodePath openNode:&needOpenNode];
    ...
    if (needOpenNode)
    {
        if ([self.delegate mutilevelTableView:self numberOfRowsInNodePath:nodePath] > 0) {
            //可以被展开
            [self.openNodePathSet addObject:nodePath];
            UITableViewRowAnimation rowAnimation = UITableViewRowAnimationNone;
            if ([self.delegate respondsToSelector:@selector(mutilevelTableView:animationOpenNode:)])
                rowAnimation = [self.delegate mutilevelTableView:self animationOpenNode:nodePath];
    ...
    {
        UITableViewRowAnimation rowAnimation = UITableViewRowAnimationNone;
        if ([self.delegate respondsToSelector:@selector(mutilevelTableView:animationDeleteNode:)])
            rowAnimation = [self.delegate mutilevelTableView:self animationDeleteNode:nodePath];
        NSMutableArray<NSIndexPath *> *indexPathList = [self indexPathArrayForNodePath:nodePath];
        //移除自己本身
        [indexPathList removeObjectAtIndex:0];
...
- (BOOL)tableView:(UITableView *)tableView canEditRowAtIndexPath:(NSIndexPath *)indexPath {
    SHMultilevelNodePath * nodePath = [self conversionToNodePathWithIndexPath:indexPath];
    if ([self.delegate respondsToSelector:@selector(mutilevelTableView:canEditRowAtNodePath:)]) {
        return [self.delegate mutilevelTableView:self canEditRowAtNodePath:nodePath];
    }
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(mutilevelTableView:commitEditingStyle:forRowAtNodePath:)]) {
        [self.delegate mutilevelTableView:self commitEditingStyle:editingStyle forRowAtNodePath:nodePath];
    }
}

- (BOOL)tableView:(UITableView *)tableView canMoveRowAtIndexPath:(NSIndexPath *)indexPath {
    SHMultilevelNodePath * nodePath = [self conversionToNodePathWithIndexPath:indexPath];
    if ([self.delegate respondsToSelector:@selector(mutilevelTableView:canMoveRowAtNodePath:)]) {
       return [self.delegate mutilevelTableView:self canMoveRowAtNodePath:nodePath];
    }
    return YES;
}
...
- (UITableViewCellEditingStyle)tableView:(UITableView *)tableView
    ...
    if ([self.delegate respondsToSelector:@selector(mutilevelTableView:editingStyleForRowAtNodePath:)]) {
        return [self.delegate mutilevelTableView:self editingStyleForRowAtNodePath:nodePath];
    }
...
sourceIndexPath toIndexPath:(NSIndexPath *)destinationIndexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(mutilevelTableView:moveRowAtNodePath:toIndexPath:)]) {
        [self.delegate mutilevelTableView:self moveRowAtNodePath:sourceNodePath toIndexPath:destinationNodePath];
    }
}
